...
npx @wp-playground/cli server --blueprint=dev/blueprint.json \
  "--mount=.:/wordpress/wp-content/plugins/copypilot-for-woocommerce" --port=9400 --login
```

The blueprint installs WooCommerce, activates the plugin, seeds five demo products, and opens **WooCommerce → CopyPilot**.
