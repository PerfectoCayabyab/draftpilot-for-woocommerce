<?php
/**
 * REST API for the CopyPilot admin app.
 *
 * @package CopyPilot
 */
...
/**
 * Routes under copypilot/v1. All routes require manage_woocommerce.
 */
class CopyPilot_REST_Controller {

	const NAMESPACE = 'copypilot/v1';
	...
	/**
	 * Only shop managers / admins may use CopyPilot.
	 *
	 * @return bool
	 */
	...
	public function generate( $request ) {
		...
		if ( '' === $tone ) {
			$tone = (string) CopyPilot_Settings::get( 'default_tone' );
		}

		$fields = array_map( 'sanitize_key', (array) $request['fields'] );
		$drafts = CopyPilot_Generator::generate( (int) $request['product_id'], $fields, $tone );
		...
		return rest_ensure_response(
			array(
				...
				'pending' => CopyPilot_Drafts::count_pending(),
			)
		);
	}
	...
	public function get_drafts( $request ) {
		...
		$drafts = CopyPilot_Drafts::list_by_status( $status );
	...
	public function decide_draft( $request ) {
		$draft = CopyPilot_Drafts::get( (int) $request['id'] );
		if ( ! $draft ) {
			return new WP_Error( 'copypilot_not_found', __( 'Draft not found.', 'copypilot-for-woocommerce' ), array( 'status' => 404 ) );
		}
		if ( 'pending' !== $draft->status ) {
			return new WP_Error( 'copypilot_decided', __( 'This draft has already been decided.', 'copypilot-for-woocommerce' ), array( 'status' => 409 ) );
		}

		if ( 'approve' === $request['decision'] ) {
			$applied = CopyPilot_Drafts::apply( $draft );
			if ( is_wp_error( $applied ) ) {
				$applied->add_data( array( 'status' => 400 ) );
				return $applied;
			}
			CopyPilot_Drafts::set_status( (int) $draft->id, 'approved' );
		} else {
			CopyPilot_Drafts::set_status( (int) $draft->id, 'rejected' );
		}
	...
	public function get_settings() {
		return rest_ensure_response(
			array(
				'settings' => CopyPilot_Settings::for_ui(),
				'tones'    => CopyPilot_Settings::tones(),
				'fields'   => CopyPilot_Drafts::fields(),
			)
		);
	}
	...
	public function update_settings( $request ) {
		CopyPilot_Settings::update( (array) $request->get_json_params() );
		return rest_ensure_response( array( 'settings' => CopyPilot_Settings::for_ui() ) );
	}
	...
	private function format_draft( $draft ) {
		...
		return array(
			...
			'product_name'   => $product ? $product->get_name() : __( '(deleted product)', 'copypilot-for-woocommerce' ),
			'field'          => $draft->field,
